    ...
    while ($arr = $db->fetch_assoc($res))
    {
        ...
        else
        {
            ...
            if ($a["lastpostread"] < $postid)
                $db->execute("UPDATE readposts SET lastpostread = ".$postid." WHERE id = ".$a["id"]) or $db->display_errors();
        }
    }
}
...
function update_topic_last_post($topicid)
{
    ...
    $res = $db->execute("SELECT id FROM posts WHERE topicid = ".$topicid." ORDER BY id DESC LIMIT 1") or $db->display_errors();
    ...
    $db->execute("UPDATE topics SET lastpost = ".$postid." WHERE id = ".$topicid) or $db->display_errors();
}
...
function get_forum_last_post($forumid)
{
    ...
    $res = $db->execute("SELECT lastpost FROM topics WHERE forumid = ".$forumid." ORDER BY lastpost DESC LIMIT 1") or $db->display_errors();
...
function insert_compose_frame($id, $newtopic = true, $quote = false)
{
    ...
    if ($newtopic)
    {
        $res = $db->execute("SELECT name FROM forums WHERE id = ".$id) or $db->display_errors();
        ...
        $res = $db->execute("SELECT * FROM topics WHERE id = ".$id) or $db->display_errors();

        $arr = $db->fetch_assoc($res)
		    or stderr(ERROR, FORUM_ERROR.TOPIC_NOT_FOUND);
    ...
    if (!$newtopic)
    {
        $postres = $db->execute("SELECT * FROM posts WHERE topicid = ".$id." ORDER BY id DESC LIMIT 10") or $db->display_errors();
        ...
        while ($post = $db->fetch_assoc($postres))
        {
            ...
            print("<tr valign='top'><td width='150' align='center' style='padding: 0px'>#" . $post["id"] . " by " . htmlsafechars($user["username"]) . "<br />" . get_date_time($post["added"]) . ($avatar != "<img src='images/default_avatar.png' width='80' title='Avatar'>" ? "<br /><img width='80' src='".$avatar."' title='Avatar'>" : "").
