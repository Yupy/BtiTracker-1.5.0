...
function dbconn($do_clean = false)
{
	...
	if ($GLOBALS["persist"]) $db = new mysqli($dbhost, $dbuser, $dbpass, $database);
...
function flag_list($with_unknown = false)
{
	...
	while ($row = $res->fetch_array(MYSQLI_BOTH)) $ret[] = $row;
	return $ret;
}
...
function check_email($email)
{
	...
	if (ereg("^([A-Za-z0-9]+_+)|([A-Za-z0-9]+\-+)|([A-Za-z0-9]+\.+)|([A-Za-z0-9]+\++))*[A-Za-z0-9]+@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([a-zA-Z0-9\-]+\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\]?)$", $email)) return true;
	else return false;
}
...
{
	$ret = "";
	for ($i = 0; $i < $len; $i++) $ret.= chr(mt_rand(0, 255));
	return $ret;
}
