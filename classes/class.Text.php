...
* Credits to Gazelle Project for the Class
*/
...
class text {
    ...
    public static function display_str($Str) {
	    if ($Str != '') {
		    $Str = misc::make_utf8($Str);
	...
	private static function smileys($Str) {
		if (count(self::$ProcessedSmileys) == 0 && count(self::$Smileys) > 0) {
			foreach (self::$Smileys as $Key => $Val) {
				self::$ProcessedSmileys[$Key] = '<img border="0" src="images/smilies/'.$Val.'" alt="" />';
			}
