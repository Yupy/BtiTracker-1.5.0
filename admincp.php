...
$mqry=run_query("select users.id from users inner join users_level on users_level.id=users.id_level where users.id=$aid and random=$arandom and admin_access='yes' and username=".sqlesc($CURUSER["username"])."");
...
if (!$CURUSER || $CURUSER["admin_access"]!="yes")
   {
    ...
    // begin the real admin page
     ...
     elseif ($do=="config" && $action=="read")
        {
            ...
            <option value="admin"<?php if($VALIDATION=="admin") echo " selected"?>>Amin</option>
            </select></td></tr>
     ...
     elseif ($do=="category" && $action=="write_add")
            {
                ...
                {
                   $name = ((isset($GLOBALS["___mysqli_ston"]) && is_object($GLOBALS["___mysqli_ston"])) ? mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_POST["name"]) : ((trigger_error("Error...", E_USER_ERROR)) ? "" : ""));
                   $sub = ((isset($GLOBALS["___mysqli_ston"]) && is_object($GLOBALS["___mysqli_ston"])) ? mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_POST["sub_category"]) : ((trigger_error("Error...", E_USER_ERROR)) ? "" : ""));
     ...
     elseif ($do=="category" && $action=="edit")
            {
                $id=intval($_GET["id"]);
                $rcat=run_query("SELECT * FROM categories WHERE id=$id") or die(((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
                $rescat=mysqli_fetch_array($rcat);
                if ($rescat)
                   {
     ...
     elseif ($do=="level" && $action=="read")
            ...
            print("<td class=\"header\" align=\"center\">".DELETE."</td>\n");
            print("</tr>\n");
            $rlevel=run_query("SELECT * from users_level ORDER BY id_level");
     ...
     elseif ($do=="level" && $action=="edit")
        {
            $id=intval($_GET["id"]);
            $rlevel=run_query("SELECT * FROM users_level WHERE id=$id");
            if (!$rlevel)
     ...
     elseif ($do=="level" && $action=="add")
        ...
        <?php
          $rlevel=run_query("SELECT DISTINCT id_level,predef_level FROM users_level ORDER BY id_level");
            ...
            if ($_POST["write"]==FRM_CONFIRM)
               {
                 ...
                 $rlevel=run_query("SELECT * FROM users_level WHERE id=$id") or die(((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
                 $level=mysqli_fetch_array($rlevel);
                 if (!$level)
                    die(GROUP_ERR_BASE_SEL);
                 $update=array();
                 $update[]="level='".((isset($GLOBALS["___mysqli_ston"]) && is_object($GLOBALS["___mysqli_ston"])) ? mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_POST["gname"]) : ((trigger_error("Error...", E_USER_ERROR)) ? "" : ""))."'";
                 $update[]="id_level='".$id."'";
                 ...
                 $id=intval($_GET["id"]);
                 run_query("INSERT INTO users_level SET $strupdate") or die(((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
            }
     ...
     elseif ($do=="level" && $action=="delete")
        {
            ...
            $rcanc=run_query("SELECT can_be_deleted FROM users_level WHERE id=$id");
            if (!$rcanc)
               die(BAD_ID);
            ...
            if ($rcancanc["can_be_deleted"]=="yes")
               {
               run_query("DELETE FROM users_level WHERE id=$id") or die(((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
               redirect("admincp.php?user=".$CURUSER["uid"]."&code=".$CURUSER["random"]."&do=level&action=read");
               }
            else
            ...
            if ($_POST["write"]==FRM_CONFIRM)
               {
                 ...
                 $strupdate=implode(",",$update);
                 $id=intval($_GET["id"]);
                 run_query("UPDATE users_level SET $strupdate WHERE id=$id") or die(((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
            }
        redirect("admincp.php?user=".$CURUSER["uid"]."&code=".$CURUSER["random"]."&do=level&action=read");
        }
     elseif ($do=="language" && $action=="read")
            ...
            print("</tr>\n");
            foreach($cat as $category) {
                         $res = run_query("SELECT * FROM users WHERE language = " . $category["id"]);
                         $total_users = 0+@mysqli_num_rows($res);
                         print("<tr>\n");
    ...
    elseif ($do=="polls" && $action=="read")
        ...
        print("&nbsp;&nbsp;<a href=\"admincp.php?user=".$CURUSER["uid"]."&code=".$CURUSER["random"]."&do=polls&action=add\"><img alt=".INSERT_NEW_POLL." border=0 src=\"images/new.gif\"></a>\n");
            ...
            print("<td class=\"header\" align=\"center\">".DELETE."</td>\n");
            print("</tr>\n");
            $res = run_query("SELECT * FROM polls ORDER BY pid");
            ?>
     ...
     elseif ($do=="polls" && $action=="updatestatus"){
        ...
        run_query("UPDATE polls SET status='true' WHERE pid='$activepoll'");
        run_query("UPDATE polls SET status='false' WHERE pid!='$activepoll'");
     ...
     elseif ($do=="polls" && $action=="edit")
            {
             $pid=intval($_GET["pid"]);
             $res =run_query("SELECT * FROM polls WHERE pid='$pid'") or die(((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
     ...
     elseif ($do=="polls" && $action=="delete")
            {
        $pid=intval($_GET["pid"]);
        run_query("DELETE FROM polls WHERE pid=$pid") or die(((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
        run_query("DELETE FROM poll_voters WHERE pid=$pid") or die(((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
        redirect("admincp.php?user=".$CURUSER["uid"]."&code=".$CURUSER["random"]."&do=polls&action=read");
            }
            ...
            if ($_POST["write"]==FRM_CONFIRM){
                ...
                if (!$res) die(ERROR.CANT_FIND_POLL);
                    ...
                    $new_poll_array=array();
                        ...
                        $question=((isset($GLOBALS["___mysqli_ston"]) && is_object($GLOBALS["___mysqli_ston"])) ? mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_POST["question"]) : ((trigger_error("Error...", E_USER_ERROR)) ? "" : ""));
                        $votings= (serialize($new_poll_array));
                        run_query("UPDATE polls SET choices='$votings', votes='$total_votes', poll_question='$question' WHERE pid='$pid'");
                        redirect("admincp.php?user=".$CURUSER["uid"]."&code=".$CURUSER["random"]."&do=polls&action=read");
            }
            }
    ...
    elseif ($do=="polls" && $action=="writenew")
            ...
            if ($_POST["write"]==FRM_CONFIRM){
                    ...
                    $startdate= time();
                    $starter=$CURUSER["uid"];
                    run_query("INSERT INTO polls SET startdate='$startdate', choices='$votings', starter_id='$starter', poll_question='$question'") or die(((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
                    redirect("admincp.php?user=".$CURUSER["uid"]."&code=".$CURUSER["random"]."&do=polls&action=read");
            }
            }
     ...
     elseif ($do=="blocks" && $action=="read")
            ...
            print("</tr>\n");
            $res = run_query("SELECT * FROM blocks ORDER BY position, status, sortid");
            ?>
     ...
     elseif ($do=="blocks" && $action=="write")
            {
                if ($_POST["write"]==FRM_CONFIRM)
                   {
                    $res = run_query("SELECT * FROM blocks");
                    while($result=mysqli_fetch_array($res)){
                        $var1= $_POST["".$result["content"]."sortid"];
                        $var2= $_POST["".$result["content"]."status"];
                        $var3= $result["blockid"];
                        $var4= $_POST["".$result["content"]."position"];
                        run_query("UPDATE blocks SET sortid='$var1', status='$var2', position='$var4' WHERE blockid='$var3'") or die(((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
                    }
                   }
     ...
     elseif ($do=="language" && $action=="delete")
            {
                ...
                if ($id!=$DEFAULT_LANGUAGE)
                   {
             $rlang=run_query("SELECT * FROM language WHERE id=$id") or die(((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
                     $reslang=mysqli_fetch_array($rlang);
     ...
     elseif ($do=="style" && $action=="read")
            ...
            print("</tr>\n");
            foreach($cat as $category) {
                         $res = run_query("SELECT * FROM users WHERE style = " . $category["id"]);
                         $total_users = 0+@mysqli_num_rows($res);
                         print("<tr>\n");
     ...
     elseif ($do=="style" && $action=="edit")
            {
                $id=intval($_GET["id"]);
                $rstyle=run_query("SELECT * FROM style WHERE id=$id") or die(((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
                $resstyle=mysqli_fetch_array($rstyle);
                if ($resstyle)
                   {
     ...
     elseif ($do=="forum" && $action=="read")
            ...
            print("<td class=\"header\" align=\"center\">".DELETE."</td>\n");
            print("</tr>\n");
            while($result=mysqli_fetch_array($resforums)) {
                         print("<tr>\n");
     ...
     elseif ($do=="forum" && $action=="edit")
            ...
            if ($what!="new")
               {
           $id=intval($_GET["id"]);
               $resforums=run_query("SELECT * FROM forums WHERE id=".$id);
               }
            if (isset($resforums) && $resforums)
               $result=mysqli_fetch_array($resforums);
            elseif ($what!="new")
                err_msg(ERROR,BAD_ID);

            block_begin(FORUM_SETTINGS);
            $rlevel=run_query("SELECT DISTINCT id_level, predef_level, level FROM users_level ORDER BY id_level");
            $alevel=array();
            while($reslevel=mysqli_fetch_array($rlevel))
                $alevel[]=$reslevel;
     ...
     elseif ($do=="forum" && $action=="saveedit")
            ...
            if ($what!="new")
               {
               $id=intval($_GET["id"]);
               run_query("UPDATE forums SET name=$name,description=$description,minclassread=$minclassread,minclasswrite=$minclasswrite,minclasscreate=$minclasscreate WHERE id=$id") or die(((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
               }
            else
                {
     ...
     elseif ($do=="forum" && $action=="delete")
            ...
            $resforum=run_query("SELECT * FROM forums WHERE id=$id");
            if ($_GET["confirm"]==1)
               {
               run_query("DELETE FROM topics WHERE forumid=$id") or die(((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
               run_query("DELETE FROM forums WHERE id=$id") or die(((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
               redirect("admincp.php?user=".$CURUSER["uid"]."&code=".$CURUSER["random"]."&do=forum&action=read");
               exit();
               }
            if ($resforum)
               {
                   $result=mysqli_fetch_array($resforum);
     ...
     elseif ($do=="banip" && $action=="read")
            ...
            $getbanned = run_query("SELECT * FROM bannedip ORDER BY added DESC") or die(((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
            ...
            if ($rowsbanned>0)
            {
               ...
               {
               $r2 = run_query("SELECT username FROM users WHERE id=$arr[addedby]") or die(((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
              ...
              else
                    {
                      ...
                      if ($firstip == -1 || $lastip == -1)
                           err_msg(ERROR,IP_ERROR);
                               ...
                               run_query("INSERT INTO bannedip (added, addedby, first, last, comment) VALUES($added, $CURUSER[uid], $firstip, $lastip, $comment)") or die(((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
     ...
     elseif ($do=="banip" && $action=="delete")
            ...
            $id = intval($_GET['ip']);
            run_query("DELETE FROM bannedip WHERE id=".$id) or die(((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
            redirect("admincp.php?user=".$CURUSER["uid"]."&code=".$CURUSER["random"]."&do=banip&action=read");
            }
     elseif ($do=="sanity" && $action=="now")
          ...
          $res=run_query("SELECT * FROM tasks");
          ...
          $sqlver=mysqli_fetch_row(run_query("SELECT VERSION()"));
