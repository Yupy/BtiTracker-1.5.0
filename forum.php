...
require("include/forum_functions.php");
...
if (isset($_GET["action"]))
    $action = $db->escape_string($_GET["action"]);
else
    $action = '';
...
if ($action == "post")
{
    ...
    if ($newtopic)
    {
        $subject = htmlsafechars(trim($subject));
        ...
        $forumid = Forum::get_topic_forum($topicid)
		    or die(ERR_TOPIC_ID);

    //------ Make sure sure user has write access in forum
    $arr = Forum::get_forum_access_levels($forumid)
	    or die(BAD_FORUM_ID);

    if ($CURUSER["id_level"] < $arr["write"] || ($newtopic && $CURUSER["id_level"] < $arr["create"]))
        stderr(ERROR, ERR_PERM_DENIED);

    $body = htmlsafechars(trim($_POST["body"]));
    ...
    if ($newtopic)
    {
        ...
        $topicid = $db->last_insert_id() or stderr(ERROR, ERR_NO_TOPIC_ID);
    } else {
        //---- Make sure topic exists and is unlocked
        $res = $db->execute("SELECT * FROM topics WHERE id = ".$topicid) or $db->display_errors();

        $arr = $db->fetch_assoc($res) or die(ERR_TOPIC_ID_NA);
    ...
    $db->execute("INSERT INTO posts (topicid, userid, added, body) " .
    "VALUES(".$topicid.", ".$userid.", ".$added.", ".$body.")") or $db->display_errors();
	
	$Memcached->delete_value('user::posts::'.$CURUSER['uid']);

    $postid = $db->last_insert_id() or die(ERR_POST_ID_NA);
    ...
    $res = $db->execute("SELECT COUNT(*) FROM posts WHERE topicid = ".$topicid) or $db->display_errors();
...
if ($action == "viewtopic")
{
    ...
    //------ Update hits column
    $db->execute("UPDATE topics SET views = views + 1 WHERE id = ".$topicid) or $db->display_errors();

    //------ Get forum
    $res = $db->execute("SELECT * FROM forums WHERE id = ".$forumid) or $db->display_errors();

    $arr = $db->fetch_assoc($res) or die("Forum = NULL");
    ...
    if ($page && $page[0] == "p")
    {
        ...
        $res = $db->execute("SELECT id FROM posts WHERE topicid = ".$topicid." ORDER BY added") or $db->display_errors();
        $i = 1;
    ...
    //------ Get posts
    $res = $db->execute("SELECT * FROM posts WHERE topicid = ".$topicid." ORDER BY id LIMIT ".$offset.", ".$perpage) or $db->display_errors();
    ...
    $pc = $db->count_select($res);

    $pn = 0;

    $r = $db->execute("SELECT lastpostread FROM readposts WHERE userid = " . intval($CURUSER["uid"]) . " AND topicid = ".$topicid) or $db->display_errors();
    ...
    if (!$lpr)
        $db->execute("INSERT INTO readposts (userid, topicid) VALUES (".$userid.", ".$topicid.")") or $db->display_errors();
    ...
    while ($arr = $db->fetch_assoc($res))
    {
        ...
        //---- Get poster details
        $res2 = $db->execute("SELECT username, level, avatar, name, flagpic FROM users INNER JOIN users_level ON users.id_level = users_level.id LEFT JOIN countries ON users.flag = countries.id WHERE users.id = ".$posterid) or $db->display_errors();
        ...
        $postername = htmlsafechars($arr2["username"]);

        if ($postername == '')
        {
            $by = 'guest';
            $avatar = '<img src="imges/default_avatar.png" title="No Avatar" width="120" />';
        } else {
            $avatar = ($arr2["avatar"] && $arr2["avatar"] != "" ? htmlsafechars($arr2["avatar"]) : "");
            $title = htmlsafechars($arr2["level"]);
            $flag = htmlsafechars($arr2['name']);
            ...
            $sql = $db->execute("SELECT * FROM posts INNER JOIN users ON posts.userid = users.id WHERE users.id = " . $posterid) or $db->display_errors();
            $posts = 0 + @$db->count_select($sql);
        ...
        if ($pn == $pc)
        {
            ...
            if ($postid > $lpr)
                $db->execute("UPDATE readposts SET lastpostread = ".$postid." WHERE userid = ".$userid." AND topicid = ".$topicid) or $db->display_errors();
        }
        ...
        {
            $res2 = $db->execute("SELECT username FROM users WHERE id = ".(int)$arr['editedby']) or $db->display_errors();

            if ($db->count_select($res2) == 1)
            {
                $arr2 = $db->fetch_assoc($res2);
                $body .= "<p><font size='1' class='small'>".LAST_EDITED_BY." <a href='userdetails.php?id=".(int)$arr['editedby']."'><b>".htmlsafechars($arr2['username'])."</b></a> at ".get_date_time($arr['editedat'])."</font></p>\n";
            }
        }
    ...
    if ($CURUSER["edit_forum"] == "yes")
    {
        ...
        $res = $db->execute("SELECT id, name, minclasswrite FROM forums ORDER BY sort, name") or $db->display_errors();
        ...
        while ($arr = $db->fetch_assoc($res))
            if ($arr["id"] != $forumid && (int)$CURUSER["id_level"] >= (int)$arr["minclasswrite"])
                print("<option value='" . (int)$arr["id"] . "'>" . htmlsafechars(unesc($arr["name"])) . "\n");
    ...
    if ($action == "movetopic")
    {
        ...
        $res = @$db->execute("SELECT minclasswrite FROM forums WHERE id = ".$forumid) or $db->display_errors();
        ...
        $res = @$db->execute("SELECT subject, forumid FROM topics WHERE id = ".$topicid) or $db->display_errors();
        ...
        if ($arr["forumid"] != $forumid)
            @$db->execute("UPDATE topics SET forumid = ".$forumid." WHERE id = ".$topicid) or $db->display_errors();

        // modifying count topics & post
        $res = @$db->execute("SELECT id FROM posts WHERE topicid = ".$topicid) or $db->display_errors();
        $numposts = @$db->count_select($res);

        $res = @$db->execute("SELECT id FROM topics WHERE forumid = ".$forumid." AND id = ".$topicid) or $db->display_errors();
        $numtopics = @$db->count_select($res);

        $db->execute("UPDATE forums SET topiccount = topiccount - ".$numtopics.", postcount = postcount - ".$numposts." WHERE id = ".(int)$arr["forumid"]) or $db->display_errors();
        $db->execute("UPDATE forums SET topiccount = topiccount + ".$numtopics.", postcount = postcount + ".$numposts." WHERE id = ".$forumid) or $db->display_errors();
    ...
    if ($action == "deletetopic")
    {
        ...
        $db->execute("DELETE FROM topics WHERE id = ".$topicid) or $db->display_errors();
        $numtopic = $db->count_affected();

        $db->execute("DELETE FROM posts WHERE topicid = ".$topicid) or $db->display_errors();
	    $Memcached->delete_value('user::posts::'.$CURUSER['uid']);
        $numposts = $db->count_affected();

        $db->execute("UPDATE forums SET topiccount = topiccount - ".$numtopic.", postcount = postcount - ".$numposts." WHERE id = ".$forumid) or $db->display_errors();
    ...
    if ($action == "editpost")
    {
        ...
        $res = $db->execute("SELECT * FROM posts WHERE id = ".$postid) or $db->display_errors();
        ...
        $res2 = $db->execute("SELECT locked FROM topics WHERE id = " . (int)$arr["topicid"]) or $db->display_errors();
        $arr2 = $db->fetch_assoc($res2);
        ...
        if ((intval($CURUSER["uid"]) != (int)$arr["userid"] || $locked) && $CURUSER["edit_forum"] != "yes")
            stderr(ERROR, ERR_PERM_DENIED);
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST')
        {
            ...
            $db->execute("UPDATE posts SET body = ".$body.", editedat = ".$editedat.", editedby = ".intval($CURUSER["uid"])." WHERE id = ".$postid) or $db->display_errors();

            $returnto = $db->escape_string($_POST["returnto"]);
    ...
    if ($action == "deletepost")
    {
        ...
        $res = $db->execute("SELECT topicid FROM posts WHERE id = ".$postid) or $db->display_errors();

        $arr = $db->fetch_row($res)
		    or stderr(ERROR, ERR_POST_NOT_FOUND);
    ...
    $res = $db->execute("SELECT id FROM posts WHERE topicid = ".$topicid." AND id < ".$postid." ORDER BY id DESC LIMIT 1") or $db->display_errors();
    ...
    else
    {
        ...
        $res2 = $db->execute("SELECT COUNT(*) FROM posts WHERE id <= ".$arr[0]." AND topicid = ".$topicid) or $db->display_errors();

        $arr2 = $db->fetch_row($res2);
    ...
    //------- Delete post
    $db->execute("DELETE FROM posts WHERE id = ".$postid) or $db->display_errors();

	$Memcached->delete_value('user::posts::'.$CURUSER['uid']);

    $numposts = $db->count_affected();

    $db->execute("UPDATE forums SET postcount = postcount - ".$numposts." WHERE id = ".$forumid) or $db->display_errors();
    ...
    if ($action == "locktopic")
    {
        ...
        $db->execute("UPDATE topics SET locked = 'yes' WHERE id = ".$topicid) or $db->display_errors();
    ...
    if ($action == "unlocktopic")
    {
        ...
        $db->execute("UPDATE topics SET locked = 'no' WHERE id = ".$topicid) or $db->display_errors();
    ...
    if ($action == "setlocked")
    {
        ...
        $db->execute("UPDATE topics SET locked = ".$locked." WHERE id = ".$topicid) or $db->display_errors();
    ...
    if ($action == "setsticky")
    {
        ...
        $db->execute("UPDATE topics SET sticky = ".$sticky." WHERE id = ".$topicid) or $db->display_errors();
    ...
    if ($action == 'renametopic')
    {
        ...
        $db->execute("UPDATE topics SET subject = ".$subject." WHERE id = ".$topicid) or $db->display_errors();
    ...
    if ($action == "viewforum")
    {
        ...
        //------ Get forum name
        $res = $db->execute("SELECT name, minclassread FROM forums WHERE id = ".$forumid) or $db->display_errors();
        ...
        $res = $db->execute("SELECT COUNT(*) FROM topics WHERE forumid = ".$forumid) or $db->display_errors();
        ...
        //------ Get topics data
        $topicsres = $db->execute("SELECT * FROM topics WHERE forumid = ".$forumid." ORDER BY sticky, lastpost DESC LIMIT ".$offset.", ".$perpage) or $db->display_errors();

        $numtopics = $db->count_select($topicsres);
        ...
        if ($numtopics > 0)
        {
            ...
            while ($topicarr = $db->fetch_assoc($topicsres))
            {
        ...
        $res = $db->execute("SELECT * FROM posts WHERE topicid = ".$topicid." ORDER BY id DESC LIMIT 1") or $db->display_errors();
        ...
        $res = $db->execute("SELECT * FROM users WHERE id = ".$lpuserid) or $db->display_errors();

        if ($db->count_select($res) == 1)
        {
            $arr = $db->fetch_assoc($res);

            $lpusername = "<a href='userdetails.php?id=".$lpuserid."'><b>".htmlsafechars($arr['username'])."</b></a>";
        }
        else
            $lpusername = 'Guest #('.$topic_userid.')';

        //------ Get author
        $res = $db->execute("SELECT username FROM users WHERE id = ".$topic_userid) or $db->display_errors();

        if ($db->count_select($res) == 1)
        {
            $arr = $db->fetch_assoc($res);

            $lpauthor = "<a href='userdetails.php?id=".$topic_userid."'><b>".htmlsafechars($arr['username'])."</b></a>";
        }
        else
            $lpauthor = 'Guest #('.$topic_userid.')';

        //---- Print row
        $r = $db->execute("SELECT lastpostread FROM readposts WHERE userid = ".$userid." AND topicid = ".$topicid) or $db->display_errors();
    ...
    if ($action == "viewunread")
    {
        ...
        $res = $db->execute("SELECT id, forumid, subject, lastpost FROM topics ORDER BY lastpost") or $db->display_errors();

        block_begin(TOPIC_UNREAD_POSTS);

        $n = 0;
        $uc = (int)$CURUSER["id_level"];
        ...
        while ($arr = $db->fetch_assoc($res))
        {
            ...
            $r = $db->execute("SELECT name, minclassread FROM forums WHERE id = ".$forumid) or $db->display_errors();
            ...
            $forumname = htmlsafechars($a['name']);
            ...
            $rescount = $db->execute("SELECT COUNT(*) FROM posts WHERE topicid = ".$topicid) or $db->display_errors();

            $arrcount = $db->fetch_row($rescount);
...
if ($action == "search")
{
    ...
    if ($keywords != '')
    {
        ...
        $res = $db->execute("SELECT COUNT(*) FROM posts WHERE MATCH (body) AGAINST (".$ekeywords.")") or $db->display_errors();
        ...
        else
        {
            ...
            $res = $db->execute("SELECT id, topicid, userid, added FROM posts WHERE MATCH (body) AGAINST (".$ekeywords.") LIMIT ".$offset.", ".$perpage) or $db->display_errors();
		    ...
		    for ($i = 0; $i < $num; ++$i)
            {
                $post = $db->fetch_assoc($res);
                $res2 = $db->execute("SELECT forumid, subject FROM topics WHERE id = ".$db->escape_string($post['topicid'])) or $db->display_errors();
                $topic = $db->fetch_assoc($res2);
                $res2 = $db->execute("SELECT name, minclassread FROM forums WHERE id = ".$db->escape_string($topic['forumid'])) or $db->display_errors();
                $forum = $db->fetch_assoc($res2);
                ...
                $res2 = $db->execute("SELECT username FROM users WHERE id = ".$db->escape_string($post['userid'])) or $db->display_errors();
                $user = $db->fetch_assoc($res2);

                if ($user["username"] == '')
                    $user["username"] = "[".(int)$post['userid']."]";

                //---- Get reply count
                $perpage = intval($CURUSER["topicsperpage"]);

                $res3 = $db->execute("SELECT COUNT(*) FROM posts WHERE id <= ".(int)$post['id']." AND topicid = ".$db->escape_string($post['topicid'])) or $db->display_errors();

                $arr3 = $db->fetch_row($res3);
  ...
  //-------- Get forums
  $forums_res = $db->execute("SELECT * FROM forums ORDER BY sort, name") or $db->display_errors();
  ...
  "<td class='header' align='center' width='15%'>".LASTPOST."</td></tr>\n");
   ...
   if ($db->count_select($forums_res) == 0)
      print("<tr><td class='lista' colspan='4' align='center'>".NO_FORUMS."</td></tr>");

  while ($forums_arr = $db->fetch_assoc($forums_res))
  {
    ...
    // Get last post info
    $post_res = $db->execute("SELECT added, topicid, userid FROM posts WHERE id = ".$lastpostid) or $db->display_errors();

    if ($db->count_select($post_res) == 1)
    {
      $post_arr = $db->fetch_assoc($post_res) or die(ERR_BAD_LAST_POST);

      $lastposterid = (int)$post_arr["userid"];
      $lastpostdate = get_date_time($post_arr["added"]);
      $lasttopicid = (int)$post_arr["topicid"];

      $user_res = $db->execute("SELECT username FROM users WHERE id = ".$lastposterid) or $db->display_errors();

      $user_arr = $db->fetch_assoc($user_res);

      $lastposter = htmlsafechars($user_arr['username']);

      $topic_res = $db->execute("SELECT subject FROM topics WHERE id = ".$lasttopicid) or $db->display_errors();

      $topic_arr = $db->fetch_assoc($topic_res);
    ...
    $res = $db->execute("SELECT COUNT(*) FROM posts WHERE topicid = ".$lasttopicid) or $db->display_errors();
    ...
    "in ".$postpages."</nobr>";
      ...
      $r = $db->execute("SELECT lastpostread FROM readposts WHERE userid = ".intval($CURUSER["uid"])." AND topicid = ".$lasttopicid) or $db->display_errors();
